...
int w,h,r,
    ...
    v,
    LX;                                      /* 故障类型 */
float b1[M][M],b0[M][M];                     /* 导纳矩阵 */
float rr,tt,C1[M][M],C0[M][M];
static float Z1[M][M],Z0[M][M];              /* 阻抗矩阵 */
static float Ip1,Ip2,Ip0;                    /* 故障点各序电流 */
static float Up1,Up2,Up0;                    /* 故障点各序电压 */
static float Zp1,Zp2,Zp0;                    /* 故障点各序阻抗 */
...
void Read_data(int argc,char **argv)         /* 读取电力系统特征文件input.txt */
{
  int i,j;
  ...
  if(fp1==NULL)
  {
    printf("你需要在当前路径创建电力系统特征文件input.txt\n内容包含节点,正负支路,注入电流数以及各序支路节点参数并用“,”相间隔\n");
  }
  fscanf(fp1,"%d,%d,%d,%d\n",&n,&m,&v,&r);   /* 输入节点,支路,注入电流数量 */
  j=1;
  do
  {
    for(i=1; i<=m+v+r; i++)
